...
import { MapPin, Edit, Trash, Plus, Search, Loader, MapPinOff } from 'lucide-react';
...
export default function LocationsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Form doğrulama - sadece name alanı zorunlu
    if (!formData.name) {
      setFormError('Lütfen mekan adını girdiğinizden emin olun.');
      return;
    }
    ...
    try {
      if (editingId) {
        ...
        // Güncelleme sonrası mekanları yeniden çek
        ...
        // Oluşturma sonrası mekanları yeniden çek
        await dispatch(fetchLocations(activeTab));
  ...
  // Handle ders yeri status toggle
  const handleToggleStatus = async (id: string, currentStatus: boolean | undefined) => {
    const statusText = currentStatus ? 'pasif' : 'aktif';
    const confirmMessage = `Bu dərs yerini ${statusText} duruma getirmek istədiyinizə əminsiniz?`;
    
    if (confirm(confirmMessage)) {
      try {
        await dispatch(toggleLocationStatus(id)).unwrap();
        
        // İşlem sonrası ders yerlerini yeniden çek
        await dispatch(fetchLocations(activeTab));
        showSnackbar(`Dərs yeri durumu ugurla ${statusText} edildi.`, 'success');
      } catch (err) {
        console.error('Dərs yeri durumu dəyişdirmə əməliyyatı baş verdi:', err);
        showSnackbar('Dərs yeri durumu dəyişdirmə əməliyyatı sırasında bir xəta baş verdi.', 'error');
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Dərs Yeri İdarəetməsi</h1>
          <p className="text-gray-600">Dərs yerlerini əlavə edin, düzəldin və idarə edin</p>
        </div>
        <button
          onClick={() => {
            setFormData(emptyForm);
      ...
      <div className="overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
        {isLoading ? (
          <div className="flex items-center justify-center p-8">
            ...
            <span className="ml-2 text-gray-600">Dərs yerləri yüklənir...</span>
          </div>
        ) : filteredLocations.length === 0 ? (
          <div className="p-6 text-center text-gray-500">
            {searchTerm 
              ? 'Axtarış kriteriyalarına uyğun dərs yeri tapılmadı.' 
              : activeTab === 'active'
                ? 'Aktiv dərs yeri tapılmadı.'
                : 'Passiv dərs yeri tapılmadı.'
            }
          </div>
        ) : (
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end space-x-2">
                        ...
                        <button
                          ...
                          title={location.isActive ? 'Passiv et' : 'Aktiv et'}
                          className="relative inline-flex h-5 w-9 cursor-pointer items-center rounded-full"
                        >
                          <span className={`${location.isActive ? 'bg-green-500' : 'bg-red-500'} absolute h-5 w-9 rounded-full transition`} />
                          <span className={`${location.isActive ? 'translate-x-5' : 'translate-x-1'} inline-block h-3 w-3 transform rounded-full bg-white transition`} />
                        </button>
                        {activeTab === 'inactive' && (
                          <button
